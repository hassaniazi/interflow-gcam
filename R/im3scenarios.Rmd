...
waterWithdrawalsByStateSectorBasin_source %>% 
  ...
  rbind(desal_totalUSA) %>% 
  rename_water_use_categories() %>% 
  plot_sankey("Water Use by Source and Category", yr = 2020) 

```

```{r fig.width=10, warning=FALSE}
# end use by technology
tech <- waterWithdrawalsByTech_watCat %>% 
  # filter(sector == "electricity") %>% 
...
waterWithdrawalsByStateSectorBasin_source %>% 
  ...
  summarise(value = sum(source_disagg)) %>% ungroup() %>% 
  select(scenario, source = subresource, target, year, value) %>% 
  rbind(desal_totalUSA, tech) %>% 
  rename_water_use_categories() %>% 
  plot_sankey("Water Use by Source, Category, and Technology")

```

```{r fig.width=10, warning=FALSE}
subsector <- waterWithdrawalsByTech_watCat %>% 
  # filter(sector == "electricity") %>% 
  group_by(scenario, watcategory, subsector, year) %>%
  # clean_cooling_tech("technology", cooling_techs, cooling_techs_years) %>% 
  summarise(value = sum(value)) %>% ungroup() %>%
  select(scenario, source = watcategory, target = subsector, year, value)
...
waterWithdrawalsByStateSectorBasin_source %>% 
  ...
  summarise(value = sum(source_disagg)) %>% ungroup() %>% 
  select(scenario, source = subresource, target, year, value) %>% 
  rbind(desal_totalUSA, subsector) %>% 
  rename_water_use_categories() %>% 
  plot_sankey("Water Use by Source, Category, and Subsector")

```

```{r fig.width=10, warning=FALSE}
sector <- waterWithdrawalsByTech_watCat %>% 
  # filter(sector == "electricity") %>% 
...
waterWithdrawalsByStateSectorBasin_source %>% 
  ...
  summarise(value = sum(source_disagg)) %>% ungroup() %>% 
  select(scenario, source = subresource, target, year, value) %>% 
  rbind(desal_totalUSA, sector) %>% 
  rename_water_use_categories() %>% 
  plot_sankey("Water Use by Source, Category, and Sector")


```

```{r fig.width=10, warning=FALSE}
elec_technology <- waterWithdrawalsByTech_watCat %>% 
  filter(sector == "electricity") %>%
  group_by(scenario, watcategory, technology, year) %>%
  # clean_cooling_tech("technology", cooling_techs, cooling_techs_years) %>% 
  summarise(value = sum(value)) %>% ungroup() %>%
...
waterWithdrawalsByStateSectorBasin_source %>% 
  ...
  summarise(value = sum(source_disagg)) %>% ungroup() %>% 
  select(scenario, source = subresource, target, year, value) %>% 
  rbind(desal_totalUSA, elec_technology) %>% 
  rename_water_use_categories() %>% 
  plot_sankey("Water Use by Source, Category, and Sector")
