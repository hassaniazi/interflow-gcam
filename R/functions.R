...
plot_sankey <- function(df_sankey, title = "GCAM-USA Sankey Diagram", tidy = T,
  ...
  p <- plot_ly(
    ...
    layout(
      title = paste0(title, " for ", scen, " scenario", if_else(animate == T, "", paste0(" in ", yr))),
      font = list(size = 11)
    )
...
S.links_mod <- data.frame(source = c(0,1,0,2,3,3, 0,1,0,2,3,3),
                      target = c(2,3,3,4,4,5, 2,3,3,4,4,5),
                      value =  c(8,4,2,8,4,2, 16,4,2,16,4,2),
...
all_regions_conus <- c("USA", states_conus, basins_conus, grid_regions_conus)
